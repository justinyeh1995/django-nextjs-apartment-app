...
const ApartmentManagement = () => {
  ...
  // Fetch all apartments on initial render
  useEffect(() => {
    fetchAllApartments();
  }, []);

  // Update the displayed apartments when the search term changes
  useEffect(() => {
    const filteredApartments = apartments.filter(apt =>
      apt.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredApartments(filteredApartments);
    setTotalPages(Math.ceil(filteredApartments.length / itemsPerPage));
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    if (filteredApartments.length < startIndex) {
      setCurrentPage(1);
      setDisplayedApartments(filteredApartments.slice(0, itemsPerPage));
      return;
    }
  ...
  const handleCreate = async (name: string) => {
    try {
      ...
      // Trigger a re-fetch of the apartments
      fetchAllApartments();
    } catch (error) {
